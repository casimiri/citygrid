-- Essential tables for user registration (safe version)
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_membership_user_org') THEN
        CREATE INDEX idx_membership_user_org ON membership(user_id, org_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_audit_log_org') THEN
